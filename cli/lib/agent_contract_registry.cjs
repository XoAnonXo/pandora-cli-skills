...
const commandContracts = [
  ...
  commandContract({
    ...
    agentWorkflow: {
      ...
      notes: [
        ...
        'If distribution flags are omitted, markets.create seeds a balanced 50/50 pool. Use --initial-yes-pct/--initial-no-pct for directional AMM probability or --yes-reserve-weight-pct/--no-reserve-weight-pct for explicit reserve allocation.',
      ],
    },
    mcp: {
  ...
  commandContract({
    ...
    agentWorkflow: {
      ...
      notes: [
        ...
        'Use --yes-reserve-weight-pct/--no-reserve-weight-pct only for explicit reserve allocation, not opening YES probability.',
    ...
    mcp: {
      ...
      inputSchema: buildInputSchema({
        flagProperties: {
          ...
          'yes-reserve-weight-pct': numberSchema('Initial YES reserve weight in percent. This controls pool allocation, not opening YES probability.', { minimum: 0, maximum: 100 }),
          'no-reserve-weight-pct': numberSchema('Initial NO reserve weight in percent. This controls pool allocation, not opening NO probability.', { minimum: 0, maximum: 100 }),
  ...
  commandContract({
    ...
    usage:
      'pandora [--output table|json] mirror browse|plan|deploy|verify|lp-explain|hedge-calc|calc|simulate|go|sync|hedge|dashboard|status|health|panic|drift|hedge-check|pnl|audit|replay|trace|logs|close ...',
    emits: ['mirror.help'],
    dataSchema: GENERIC_DATA_SCHEMA_REF,
  }),
  commandContract({
    ...
    mcp: {
      ...
      inputSchema: buildInputSchema({
        flagProperties: {
          ...
          'indexer-url': commonFlags.indexerUrl,
          'timeout-ms': commonFlags.timeoutMs,
          'drift-trigger-bps': integerSchema('Drift trigger in basis points used when projecting LP hedge posture.', { minimum: 1 }),
          'hedge-trigger-usdc': numberSchema('Hedge trigger size in USDC used when projecting LP hedge posture.', { minimum: 0 }),
          'hedge-ratio': numberSchema('Desired hedge ratio.', { minimum: 0 }),
          ...
          'no-hedge': booleanSchema('Disable the hedge leg from the plan.'),
          'skip-initial-hedge': booleanSchema('Preserve startup hedge baseline behavior in the plan output.'),
          ...
          'flashbots-relay-url': stringSchema('Optional Flashbots/private relay URL for the LP hedge bundle path.'),
          'flashbots-auth-key': stringSchema('Optional Flashbots auth key or signer reference for the LP hedge bundle path.'),
    ...
    mcp: {
      ...
      inputSchema: buildInputSchema({
        flagProperties: {
          ...
          'hedge-ratio': numberSchema('Desired hedge ratio.', { minimum: 0 }),
          'hedge-scope': enumSchema(['pool', 'total'], 'Hedge basis. total includes held Pandora outcome tokens in addition to pool reserves; pool hedges only the AMM reserves.'),
          'no-hedge': booleanSchema('Disable the hedge leg from the bundle.'),
          'skip-initial-hedge': booleanSchema('Preserve startup hedge baseline behavior in the bundle output.'),
          'max-hedge-usdc': numberSchema('Maximum hedge notional in USDC.', { minimum: 0 }),
          'max-open-exposure-usdc': numberSchema('Maximum open exposure in USDC.', { minimum: 0 }),
          'cooldown-ms': integerSchema('Idempotency cooldown in milliseconds.', { minimum: 1 }),
          'depth-slippage-bps': integerSchema('Depth slippage in basis points.', { minimum: 1, maximum: 10000 }),
    ...
    mcp: {
      ...
      inputSchema: buildInputSchema({
        ...
        flagProperties: {
          ...
          paper: commonFlags.paper,
          'dry-run': commonFlags.dryRun,
          'execute-live': booleanSchema('Execute live hedge actions.'),
          execute: commonFlags.execute,
          'private-key': commonFlags.privateKey,
          'profile-id': commonFlags.profileId,
          'profile-file': commonFlags.profileFile,
          ...
          'chain-id': commonFlags.chainId,
          'rpc-url': commonFlags.rpcUrl,
          'polymarket-rpc-url': stringSchema('Polygon RPC URL for Polymarket preflight; comma-separated fallbacks are tried in order.'),
          'trust-deploy': booleanSchema('Trust manifest deploy pair.'),
          'indexer-url': commonFlags.indexerUrl,
          'timeout-ms': commonFlags.timeoutMs,
          ...
          'interval-ms': integerSchema('Hedge interval in milliseconds.', { minimum: 1 }),
          iterations: integerSchema('Maximum tick iterations before exit.', { minimum: 1 }),
          'adopt-existing-positions': booleanSchema('Seed managed Polymarket inventory from existing balances.'),
          ...
          'hedge-ratio': numberSchema('Desired hedge ratio.', { minimum: 0 }),
          'hedge-scope': enumSchema(['pool', 'total'], 'Hedge basis. total includes held Pandora outcome tokens in addition to pool reserves; pool hedges only the AMM reserves.'),
          'no-hedge': booleanSchema('Disable the hedge leg.'),
          'skip-initial-hedge': booleanSchema('Suppress the first hedge at startup.'),
          ...
          'rebalance-route': buildMirrorRebalanceRouteSchema(),
          'rebalance-route-fallback': buildMirrorRebalanceRouteFallbackSchema(),
          'flashbots-relay-url': stringSchema('Optional Flashbots/private relay URL for the LP hedge execution leg.'),
          'flashbots-auth-key': stringSchema('Optional Flashbots auth key or signer reference for the LP hedge execution leg.'),
          'flashbots-target-block-offset': integerSchema('Optional target block offset for Flashbots/private bundle submission.', { minimum: 1 }),
          'polymarket-host': stringSchema('Polymarket host override.'),
          'polymarket-gamma-url': stringSchema('Polymarket Gamma API base URL.'),
          'polymarket-gamma-mock-url': stringSchema('Polymarket Gamma mock URL.'),
        ...
        anyOf: mirrorSyncSelectorAndOptionalModeOneOf,
  ...
  commandContract({
    name: 'mirror.hedge.status',
    summary: 'Inspect detached LP hedge daemon status and health metadata.',
    usage: 'pandora [--output table|json] mirror hedge status --pid-file <path>|--strategy-hash <hash>',
    emits: ['mirror.hedge.status', 'mirror.hedge.status.help'],
    dataSchema: '#/definitions/MirrorHedgePayload',
    mcpExposed: true,
    mcp: {
      command: ['mirror', 'hedge', 'status'],
      description: 'Inspect detached LP hedge daemon status and health metadata.',
      inputSchema: buildInputSchema({
        flagProperties: {
  ...
  commandContract({
    name: 'mirror.hedge.stop',
    summary: 'Stop detached LP hedge daemon.',
    usage: 'pandora [--output table|json] mirror hedge stop --pid-file <path>|--strategy-hash <hash>|--market-address <address>|--all',
    emits: ['mirror.hedge.stop', 'mirror.hedge.stop.help'],
    dataSchema: '#/definitions/MirrorHedgePayload',
    mcpExposed: true,
    mcp: {
      command: ['mirror', 'hedge', 'stop'],
      description: 'Stop detached LP hedge daemon.',
      inputSchema: buildInputSchema({
        includeIntent: true,
        flagProperties: {
          'pid-file': stringSchema('Mirror daemon pid file path.'),
          'strategy-hash': stringSchema('Mirror strategy hash.'),
          'market-address': commonFlags.marketAddress,
          all: booleanSchema('Stop all LP hedge daemons.'),
        },
    ...
    mcp: {
      ...
      inputSchema: buildInputSchema({
        ...
        flagProperties: {
          ...
          'hedge-ratio': numberSchema('Desired hedge ratio.', { minimum: 0 }),
          'hedge-scope': enumSchema(['pool', 'total'], 'Hedge basis. total includes held Pandora outcome tokens in addition to pool reserves; pool hedges only the AMM reserves.'),
          'skip-initial-hedge': booleanSchema('Capture the startup hedge gap as a baseline and only hedge later delta changes. This does not change the meaning of hedge-scope.'),
          verbose: booleanSchema('Emit expanded per-tick console diagnostics.'),
