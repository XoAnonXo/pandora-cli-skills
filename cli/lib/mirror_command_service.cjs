  ...
  return async function runMirrorCommand(args, context) {
    ...
    if (!action || action === '--help' || action === '-h') {
      if (context.outputMode === 'json') {
        ...
        console.log('  For AMM deploy/go flows, prefer --initial-yes-pct/--initial-no-pct to set the opening probability directly. Raw --distribution-yes-pct/--distribution-no-pct remain reserve-weight controls.');
