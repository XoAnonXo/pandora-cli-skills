...
module.exports = async function handleMirrorDeploy({ shared, context, deps }) {
  ...
  const notes = [
    ...
    'For AMM deploy flows, prefer --initial-yes-pct/--initial-no-pct to set the opening probability directly. Raw --distribution-yes-pct/--distribution-no-pct remain reserve-weight controls.',
  ];
