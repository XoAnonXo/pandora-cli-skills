...
function renderMirrorHedgeCheckTable(data) {
  const summary = data.summary || {};
  renderKeyValueRows('Mirror Hedge Check', [
    ['strategyHash', data.strategyHash || ''],
    ['stateFile', data.stateFile || ''],
    ['targetHedgeShares', summary.targetHedgeShares],
    ['currentHedgeShares', summary.currentHedgeShares],
    ['hedgeGapShares', summary.hedgeGapShares],
    ['triggerShares', summary.triggerShares],
    ['triggered', summary.triggered ? 'yes' : 'no'],
    ['hedgeSide', summary.hedgeSide || ''],
    ['runtimeHealth', summary.runtimeHealth || ''],
  ]);
}

module.exports = async function handleMirrorHedgeCheck({ actionArgs, shared, context, deps }) {
  const {
    CliError,
    includesHelpFlag,
    emitSuccess,
  ...
  if (includesHelpFlag(actionArgs)) {
    if (context.outputMode === 'json') {
      emitSuccess(
        context.outputMode,
        'mirror.hedge-check.help',
        commandHelpPayload(usage, [
          'mirror hedge-check is the dedicated live hedge-gap/readiness surface backed by the same selector-first data path as mirror status and mirror pnl.',
          'Use it when operators need the current hedge target, gap, trigger state, and Polymarket inventory context without parsing the full dashboard payload.',
        ]),
      );
    } else {
      console.log(`Usage: ${usage}`);
      console.log('mirror hedge-check is the dedicated live hedge-gap/readiness surface backed by the same selector-first data path as mirror status and mirror pnl.');
      console.log('Use it when operators need the current hedge target, gap, trigger state, and Polymarket inventory context without parsing the full dashboard payload.');
    }
    return;
  }
  ...
  if (!selector.pandoraMarketAddress) {
    throw new CliError(
      ...
      'mirror hedge-check requires --pandora-market-address/--market-address (or a state file containing it).',
    );
  }
  if (!selector.polymarketMarketId && !selector.polymarketSlug) {
    throw new CliError(
      'MISSING_REQUIRED_FLAG',
      'mirror hedge-check requires --polymarket-market-id or --polymarket-slug (or a state file containing one).',
    );
  }
  ...
  try {
    ...
    throw coerceMirrorServiceError(err, 'MIRROR_HEDGE_CHECK_FAILED');
  }
