...
module.exports = async function handleMirrorSync({ shared, context, deps, mirrorSyncUsage }) {
  ...
  const helpNotes = [
    ...
    'Pandora Mirroring Mode means `mirror sync --no-hedge`: Pandora follows Polymarket odds and the loop does not place a Polymarket hedge.',
    'If you want delta-neutral hedging on Polymarket instead, use `mirror hedge`, which is Polymarket Hedge Mode.',
    'Plain `mirror sync` without `--no-hedge` is the hybrid path: it may rebalance Pandora and hedge on Polymarket in the same loop.',
