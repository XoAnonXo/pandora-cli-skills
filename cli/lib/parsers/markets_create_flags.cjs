...
function finalizeDistribution(options, CliError) {
  ...
  if (options.distributionYes === null && options.distributionNo === null) {
    ...
    options.distributionInputMode = 'default-balanced';
  } else if (!options.distributionInputMode) {
    options.distributionInputMode = 'raw-reserve-units';
  }
}
...
function createParseMarketsCreateFlags(deps) {
  ...
  return function parseMarketsCreateFlags(args) {
    ...
    for (let i = 0; i < rest.length; i += 1) {
      ...
      if (token === YES_RESERVE_WEIGHT_PCT_FLAG) {
        options.yesReserveWeightPct = parseDistributionPercent(
          requireFlagValue(rest, i, YES_RESERVE_WEIGHT_PCT_FLAG),
          YES_RESERVE_WEIGHT_PCT_FLAG,
          CliError,
        );
        i += 1;
      ...
      if (token === NO_RESERVE_WEIGHT_PCT_FLAG) {
        options.noReserveWeightPct = parseDistributionPercent(
          requireFlagValue(rest, i, NO_RESERVE_WEIGHT_PCT_FLAG),
          NO_RESERVE_WEIGHT_PCT_FLAG,
          CliError,
        );
        i += 1;
      ...
      if (token === '--initial-yes-pct') {
        options.initialYesPct = parseDistributionPercent(
          requireFlagValue(rest, i, '--initial-yes-pct'),
          '--initial-yes-pct',
          CliError,
        );
        i += 1;
      ...
      if (token === '--initial-no-pct') {
        options.initialNoPct = parseDistributionPercent(
          requireFlagValue(rest, i, '--initial-no-pct'),
          '--initial-no-pct',
          CliError,
        );
        i += 1;
    ...
    if (action === 'plan') {
      ...
      assertNoDeployRoutePlanFlags(deployRoutePlanFlags, 'markets create plan', CliError);
