...
- Use `paper-mirror` / `live-mirror` onboarding goals for Pandora Mirroring Mode, and `paper-hedge-daemon` / `live-hedge-daemon` for Polymarket Hedge Mode.
...
| Polymarket Hedge Mode | `mirror hedge ...`, `dashboard`, `mirror dashboard`, `mirror status`, `mirror health`, `mirror panic`, `mirror logs`, `mirror replay`, `mirror audit`, `mirror pnl`, `fund-check` | Use `mirror hedge` when Pandora flow should be hedged on Polymarket and you want to stay delta neutral while earning Pandora fees. Bundle artifacts support DigitalOcean droplets and generic VPS targets today. Cloudflare Workers are not supported in v1 because the daemon expects a long-running stateful runtime with local lifecycle. |
| Pandora Mirroring Mode and hybrid sync | `mirror sync once|run|start|stop|status`, `mirror drift`, `mirror hedge-check`, `mirror calc`, `mirror close`, `polymarket check|approve|preflight|balance|deposit|withdraw|trade` | Use `mirror sync --no-hedge` for Pandora Mirroring Mode when Pandora should follow Polymarket odds without placing Polymarket hedges. Plain `mirror sync` without `--no-hedge` is the hybrid loop for local/manual execution and troubleshooting. Sync remains separate-leg, not atomic; payloads expose `reserveSource` and rebalance sizing truth. Use `mirror sync status` for daemon health on the local sync surface, `mirror drift` or `mirror hedge-check` for narrower actionability views, `mirror calc` for exact target-percentage sizing, `polymarket balance|deposit` for signer/proxy funding on Polygon USDC.e, and treat `polymarket withdraw` execute mode as signer-controlled only unless the proxy itself submits the transfer. |
