...
| Agent-native integration | `bootstrap`, `capabilities`, `schema`, `policy list|get|lint|explain|recommend`, `profile list|get|explain|recommend|validate`, `mcp`, `explain`, `agent market autocomplete`, `agent market validate` | Start with `bootstrap`; open `agent-interfaces.md` for exact envelope and MCP details. |
