...
- `mirror hedge status` now surfaces sell retry counters and operator warnings for stuck reductions.
  - use `sellRetryAttemptedCount`, `sellRetryBlockedCount`, `sellRetryFailedCount`, and `sellRetryRecoveredCount` to understand whether sell-side reductions are progressing
  - `BOTH_SIDE_INVENTORY_LOCKUP` means actual YES/NO inventory is still dual-sided while the target is single-sided
...
pandora [--output table|json] markets create plan|run [flags]
pandora [--output table|json] markets hype plan|run [flags]
...
pandora [--output table|json] agent market hype|autocomplete|validate [flags]
...
Mirror AMM flag migration:
- `--distribution-yes-pct` and `--distribution-no-pct` are retired.
- Use `--initial-yes-pct` / `--initial-no-pct` for opening probability.
- Use `--yes-reserve-weight-pct` / `--no-reserve-weight-pct` for explicit reserve weights.

Mirror sync startup hedge note:
- `--skip-initial-hedge` captures the startup hedge gap as a baseline and only hedges later delta changes.
- It does not change `--hedge-scope pool|total`; it only suppresses the startup baseline hedge for that state file.
...
- `markets hype plan` is the canonical fresh-news market planning surface.
...
- `mirror status --with-live` adds `crossVenue`, `hedgeStatus`, `actionability`, `actionableDiagnostics`, `pnlScenarios`, `verifyDiagnostics`, `polymarketPosition.balanceScope`, `polymarketPosition.mergeReadiness`, `polymarketPosition.diagnostics`, `sourceMarket`, `pandoraMarket`, `netPnlApproxUsdc`, `pnlApprox`, and `netDeltaApprox`, and degrades partial live visibility into diagnostics instead of hard failures.
...
preflight [--condition-id <id>|--slug <slug>|--token-id <id>] [--token yes|no] [--amount-usdc <n>] [--shares <n> for sell] [--side buy|sell] [--polymarket-host <url>] [--polymarket-mock-url <url>] [--timeout-ms <ms>] [--fork] [--fork-rpc-url <url>] [--fork-chain-id <id>] [--rpc-url <url>] [--private-key <hex>] [--funder <address>]
