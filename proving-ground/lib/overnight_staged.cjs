...
function buildPlannerPrompt(options) {
  ...
  return {
    systemPrompt: [
      ...
      'Do not choose source targets listed in blocked_source_targets.',
    ...
    userPrompt: JSON.stringify({
      ...
      blocked_source_targets: truncateList(options.blockedSourceTargets || [], 8).map((entry) => ({
        id: entry.id,
        reasonCode: entry.reasonCode,
        summary: entry.summary,
      })),
...
function buildEditorReturnShape(windows) {
  return {
    ...
    source_edit: {
      edit_mode: 'full_window | subrange',
      ...
      start_line: `required only when edit_mode is subrange; stay inside ${windows.sourceWindow.startLine}-${windows.sourceWindow.endLine}`,
      end_line: `required only when edit_mode is subrange; stay inside ${windows.sourceWindow.startLine}-${windows.sourceWindow.endLine}`,
    ...
    test_edit: {
      ...
      start_line: `required only when edit_mode is subrange; stay inside ${windows.testWindow.startLine}-${windows.testWindow.endLine}`,
      end_line: `required only when edit_mode is subrange; stay inside ${windows.testWindow.startLine}-${windows.testWindow.endLine}`,
      operation: 'optional; defaults to replace_block',
    },
    logical_explanation: {
...
function buildEditorPrompt(options) {
  return {
    systemPrompt: [
      ...
      'Use edit_mode full_window for the normal case. In full_window mode, omit start_line and end_line and replace the entire provided window exactly.',
      'Use edit_mode subrange only when the provided window contains unrelated surrounding lines that must stay untouched.',
    ...
    userPrompt: JSON.stringify({
      ...
      editor_rules: [
        ...
        'Prefer edit_mode full_window when you can safely replace the full provided window.',
        'In edit_mode full_window, omit start_line and end_line and include unchanged surrounding lines that still belong inside the window.',
        'In edit_mode subrange, include start_line and end_line inside the same provided window.',
      ],
  ...
  return {
    ...
    userPrompt: JSON.stringify({
      ...
      repair_rules: [
        ...
        'Use edit_mode full_window when the repaired replacement covers the full provided window.',
        'Use edit_mode subrange only when you must preserve unrelated surrounding lines inside the same window.',
