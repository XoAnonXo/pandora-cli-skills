...
test("Fork-Based E2E Market Maker Integration Tests", { timeout: 600_000 }, async (t) => {
	...
	await t.test("Phase 2c: mirror deploy --execute on Anvil fork", async () => {
		...
		if (fs.existsSync(deployGuardDir)) {
			for (const f of fs.readdirSync(deployGuardDir)) {
				if (f.endsWith(".json")) {
					try {
						fs.unlinkSync(path.join(deployGuardDir, f));
					} catch {
						/* ignore */
					}
				}
	...
	await t.test("Phase 4c: second sync tick — cumulative state", async () => {
		...
		const stateData = JSON.parse(fs.readFileSync(stateFile, "utf8"));
		assert.ok(stateData.tickCount === undefined || stateData.tickCount >= 1, "State should track cumulative ticks");
	});
	...
	await t.test("Phase 5a: auto-withdraw on expiry via evm_increaseTime", async () => {
		...
		if (result.status === 0 && payload && payload.ok) {
			...
			const hasCloseSignal =
				syncData.autoWithdraw ||
				syncData.closedMarketDetected ||
				syncData.skipReason === "market-closed" ||
				syncData.skipReason === "past-close";
			assert.ok(
	...
	await t.test("Phase 6a: resolve --execute on fork", async () => {
		...
		if (result.status === 0) {
			...
			const isSimulationError =
				errorCode === "RESOLVE_EXECUTION_FAILED" || output.includes("simulation") || output.includes("revert");

			assert.ok(
				isAuthError || isSimulationError,
	...
	await t.test("Phase 7a: trade buy rejects INSUFFICIENT_USDC_BALANCE", async () => {
		...
		try {
			...
			const encoded = encodeAbiParameters(parseAbiParameters("address, uint256"), [ANVIL_ACCOUNT_0, 9n]);
			const balanceSlot = keccak256(encoded);
			await jsonRpc(ANVIL_RPC, "anvil_setStorageAt", [USDC_ADDRESS, balanceSlot, "0x" + "0".padStart(64, "0")]);
			...
			assert.ok(
				code === "INSUFFICIENT_USDC_BALANCE" ||
					...
					code === "APPROVE_SIMULATION_FAILED",
	...
	await t.test("Phase 7e: sell rejects INSUFFICIENT_OUTCOME_TOKEN_BALANCE", async () => {
		...
		const code = payload.error.code;
		assert.ok(
			code === "INSUFFICIENT_OUTCOME_TOKEN_BALANCE" ||
				code === "OUTCOME_TOKEN_BALANCE_READ_FAILED" ||
				code === "OUTCOME_TOKEN_ADDRESS_UNAVAILABLE" ||
				code === "TRADE_EXECUTION_FAILED",
	...
	await t.test("Phase 7f: lp add rejects with insufficient USDC", async () => {
		...
		try {
			...
			const code = payload.error.code;
			assert.ok(
				code === "LP_ADD_APPROVE_SIMULATION_FAILED" ||
					code === "LP_ADD_SIMULATION_FAILED" ||
					code === "LP_ADD_APPROVE_EXECUTION_FAILED" ||
					code === "LP_ADD_EXECUTION_FAILED" ||
	...
	await t.test("Phase 7g: lp remove --all handles LP token presence correctly", async () => {
		...
		try {
			...
			assert.equal(
				secondPayload.error?.code,
				"LP_REMOVE_ZERO_BALANCE",
	...
	await t.test("Phase 7j: mirror sync --execute-live blocks when kill-switch file exists", async () => {
		...
		try {
			...
			assert.equal(
				result.status,
				0,
				`Sync with kill-switch should still exit 0 (graceful stop): ${result.output}`,
			);
			const payload = parseJsonOutput(result);
			assert.equal(payload.ok, true);
	...
	await t.test("Phase 7k: mirror deploy --execute without validation ticket rejects", async () => {
		...
		const code = payload.error?.code;
		assert.ok(
			code === "MIRROR_VALIDATION_REQUIRED" ||
				code === "MIRROR_VALIDATION_MISMATCH" ||
				code === "INVALID_ARGS" ||
				code === "DEPLOY_GUARD_MISSING_TICKET",
	...
	await t.test("Phase 8a: resolve uses factory->isOperator fallback", async () => {
		...
		const result = await runCliAsync(
			[
				...
				"yes",
				"--reason",
				"Testing factory->isOperator resolve path",
				"--dry-run",
				"--fork-rpc-url",
				ANVIL_RPC,
				"--fork-chain-id",
		...
		if (payload.ok && payload.data?.precheck) {
			...
			if (hasRole) {
				assert.ok(
					payload.data.txPlan?.functionName,
	...
	await t.test("Phase 8b: trade succeeds under high gas price", async () => {
		...
		try {
			...
			await jsonRpc(ANVIL_RPC, "anvil_setNextBlockBaseFeePerGas", ["0x" + (500n * 10n ** 9n).toString(16)]);
			await jsonRpc(ANVIL_RPC, "evm_mine", []);
